...
%set files for vowel tracking; similarly, set files for pitch tracking
if isfield(expt,'trackingFileName')
    ...
    ostFN = which('measureFormantsWorking.ost'); %for example: measurePitchWorking.ost and measurePitchWorking.pcf
...
% set noise: in the pitch shifting paradigm we are not adding noise, so we
% can remove this
w = get_noiseSource(p);
...
for itrial = 1:length(trials2run)  % for each trial
    ...
    while ~bGoodTrial
        ...
        subjOstParams = get_ost('experiment_helpers', 'measureFormants', 'full', 'working'); %change this in measurePitch
