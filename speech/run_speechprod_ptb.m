function [expt] = run_speechprod_ptb(snum,dataPath,usetrigs,practicemode,manualmode)
...
if practicemode
    nbreps = 2;
    nblocks = 2;
    trigoffset = length(words)*2;
else
    nbreps = 10;                          % repetitions per block
    nblocks = 5;                          % number of blocks
    trigoffset = 0;
end

nbtrials = length(words)*nbreps;      % number of trials per block
ntrials = nbtrials * nblocks;         % total number of trials in expt
stimdur = 1;                          % time stim is on screen, in seconds
interstimdur = .75;                   % minimum time between stims, in seconds
interstimjitter = .75;                % maximum extra time between stims (jitter)

wordbank = repmat(1:length(words),1,nbreps);
