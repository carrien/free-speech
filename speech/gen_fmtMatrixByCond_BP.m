...
for c = 1:length(indShift) % for each condition to plot
    ...
    if indShift(c).inds
        ...
        if isfield(indShift,'shiftind')
            ...
            onsetMeanProj = nanmean(proj.(conds{c})(floor(fs*0.025):ceil(fs*0.1),:),1);
            normProj.(conds{c}) = proj.(conds{c}) - onsetMeanProj;
            normProj_mean.(conds{c}) = nanmean(normProj.(conds{c}),2);
        end
...
if isfield(indShift,'shiftind')
    ...
    fmtMatrix.normProj = normProj; fmtMeans.normProj = normProj_mean;
