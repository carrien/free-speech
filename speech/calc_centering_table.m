function [centering_table_pph,anti_table,centering_table_all] = calc_centering_table(basepath,subs,fdatafile,subFolders,subConds)
...
% for example:
% centering_table = calc_centering_table('/Volumes/bits/projectdata',{'TMA'
%     'JL' 'CN' 'SB'},[],{'pre' 'post'})

if nargin < 3 || isempty(fdatafile), fdatafile = 'fdata_vowel'; end
if nargin < 4 || isempty(subFolders), subFolders = {1}; end
if nargin < 5 || isempty(subConds), subConds = {1}; end
...
for s=1:length(dataPaths)
    for f = 1:length(subFolders)
        ...
        if stage == 1
            ...
            dP = dataPaths{s}; % if there are multiple stages
        end
        
        for sc = 1:length(subConds)
            subc = subConds{sc}
            if subc == 1
                if exist('subDataPath','var')
                    subDataPath = subDataPath; % if there are no stages or subconds
                else
                    subDataPath = fullfile(dP,stage);
                end
            else
                subDataPath = fullfile(dP,stage,subc)
            end
            ...
            for v=1:length(vowels)
                ...
                [centering,group] = add2anovars(centering,data2add',group,s,v,f,sc);
                ...
                [centering_rev,groupcentrev] = add2anovars(centering_rev,data2add',groupcentrev,s,v,f,sc);
                ...
                [anticentering,groupanti] = add2anovars(anticentering,data2add',groupanti,s,v,f,sc);
                ...
                [anticentering_rev,groupantirev] = add2anovars(anticentering_rev,data2add',groupantirev,s,v,f,sc);
                ...
                [inidists,groupinidists] = add2anovars(inidists,data2add',groupinidists,s,v,f,sc);
                %         means.dinit = [means.dinit mean(data2add)];
                %         % d_mid, all trials (group 2)
                data2add = fmtdata.mels.(vow).mid50p.dist;
                [middists,groupmiddists] = add2anovars(middists,data2add',groupmiddists,s,v,f,sc);
                %         means.dmid = [means.dmid mean(data2add)];
                %
                %         % f0 init, all trials (group 1)
                data2add = f0data.mels.(vow).first50ms.rawavg.f0;
                [f0s,groupf0s] = add2anovars(f0s,data2add',groupf0s,s,v,f,sc);
                %         % f0 mid, all trials (group 2)
                data2add = f0data.mels.(vow).mid50p.rawavg.f0;
                [f0s_rev,groupf0srev] = add2anovars(f0s_rev,data2add',groupf0srev,s,v,f,sc);
                %
            end
        end
    end
...
subconds = group{4};
noisecond = {subConds{subconds}}';

centering_table_pph = table(centering, subj,vowel,stage,noisecond);

% took out centering_rev,
% anticentering
...
antisubconds = groupanti{4};
noisecond = {subConds{antisubconds}}';

anti_table = table(anticentering, subj, vowel,stage,noisecond); % anti-centering uses DIFFERENT TRIALS from peripheral centering
...
allsubconds = groupinidists{4};
noisecond = {subConds{allsubconds}}';

loginidists = log(inidists);
logmiddists = log(middists);
trialdists = inidists-middists; % acoustic distance traveled during one trial

centering_table_all = table(subj, vowel, stage, noisecond, trialdists, inidists, middists, loginidists, logmiddists, f0s, f0s_rev);
